const Fundamentals = {
  ...
  _thresholdTip(status, metric, sector, median) {
    if (!status) return `${metric} — no sector benchmark`;
    const labels = { green: 'Favorable', yellow: 'In line', red: 'Unfavorable' };
    const medianStr = median != null ? ` (${Currency.formatNum(median, 1)})` : '';
    return `${labels[status] || status} vs ${sector} median${medianStr}`;
  },
  ...
  _renderRow(r) {
    ...
    return `
      <tr class="border-b border-surface-border/50 hover:bg-surface-secondary/50">
        ...
        <td class="py-2 px-2 text-right relative ${Currency.thresholdClass(th.pe)}">${Currency.formatNum(r.pe, 1)}<span class="tip">${this._thresholdTip(th.pe, 'P/E', sec, med.pe)}</span></td>
        ...
        <td class="py-2 px-2 text-right relative ${Currency.thresholdClass(th.roe)}">${Currency.formatNum(r.roe, 1)}<span class="tip">${this._thresholdTip(th.roe, 'ROE', sec, med.roe)}</span></td>
        ...
        <td class="py-2 px-2 text-right relative ${Currency.thresholdClass(th.debt_to_equity)}">${Currency.formatNum(r.debt_to_equity, 1)}<span class="tip">${this._thresholdTip(th.debt_to_equity, 'D/E', sec, med.debt_to_equity)}</span></td>
        <td class="py-2 px-2 text-right relative ${Currency.thresholdClass(th.profit_margin)}">${Currency.formatNum(r.profit_margin, 1)}<span class="tip">${this._thresholdTip(th.profit_margin, 'Margin', sec, med.profit_margin)}</span></td>
