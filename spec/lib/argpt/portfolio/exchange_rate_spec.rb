...
RSpec.describe Argpt::Portfolio::ExchangeRate do
  ...
  describe ".best_mep" do
    it "prefers AL30 when present" do
      rate = Argpt::Portfolio::ExchangeRate.best_mep(rates_with_al30)

      expect(rate.ticker).to eq("AL30")
    end

    it "falls back to first entry when AL30 is absent" do
      rate = Argpt::Portfolio::ExchangeRate.best_mep(rates_without_al30)
    ...
    it "raises on empty data" do
      expect { Argpt::Portfolio::ExchangeRate.best_mep([]) }
        .to raise_error(Argpt::Error, /no rate data/i)
    end
  end

  describe ".best_ccl" do
    it "prefers AL30 when present" do
      rate = Argpt::Portfolio::ExchangeRate.best_ccl(rates_with_al30)

      expect(rate.ticker).to eq("AL30")
    end

    it "falls back to first entry when AL30 is absent" do
      rate = Argpt::Portfolio::ExchangeRate.best_ccl(rates_without_al30)
    ...
    it "maps keys correctly" do
      ...
      expect(rate.bid).to eq(1180.50)
      expect(rate.ask).to eq(1185.00)
      expect(rate.mark).to eq(1182.75)
    end

    it "raises on empty data" do
      expect { Argpt::Portfolio::ExchangeRate.best_ccl([]) }
        .to raise_error(Argpt::Error, /no rate data/i)
    end
  end
